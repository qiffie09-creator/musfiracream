...
export async function syncFirebaseWithInitialData() {
  ...
  try {
    ...
    if (!firestore) {
      console.log('Firebase Admin Firestore not available for initial seed, using local persistence.');
      return;
    }
    ...
    if (!settingsDoc.exists) {
      ...
      console.log('Firebase Firestore successfully seeded with complete Musfira Beauty Cream catalog, reviews, settings, and orders!');
    }
    isSeeded = true;
  } catch (err) {
    console.warn('Firebase auto-seed warning (graceful fallback active):', err);
  }
}
